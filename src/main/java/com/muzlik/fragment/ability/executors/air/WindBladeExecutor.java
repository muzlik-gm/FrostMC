...
/**
 * Wind Blade - Air Fragment Primary Ability
 ...
 */
public class WindBladeExecutor implements AbilityExecutor {
    @Override
    public void execute(AbilityContext context) {
        ...
        // 5-Layer VFX System - Swift blade aesthetic
        VFXLayerBuilder vfxBuilder = new VFXLayerBuilder(plugin, eyeLoc, rank, player)
            .withPerformanceManager(plugin.getVFXPerformanceManager())
            // Core: CLOUD blade - clean linear trail
            .core(Particle.CLOUD, coreCount, ParticlePattern.LINE, spread, spread, spread, 0.08 + (rank * 0.01), null)
            // Secondary: END_ROD trail - minimal shimmer
            .secondary(Particle.END_ROD, secondaryCount, ParticlePattern.POINT, spread * 0.5, spread * 0.5, spread * 0.5, 0.04, null)
        ...
        if (rank >= 2) {
            vfxBuilder.cinematic(0.15 + (rank * 0.03), CinematicEffect.WIND_DISTORTION);
        }
